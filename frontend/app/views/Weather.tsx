  ...
  return (
    <div className="animate-fadeIn space-y-8">
      ...
      <Card>
        ...
        <div className="custom-scrollbar flex overflow-x-auto gap-2 pb-2 -mx-6 px-6 [&::-webkit-scrollbar]:hidden md:[&::-webkit-scrollbar]:block [-ms-overflow-style:auto] [scrollbar-width:thin]">
          {hourly.map((hour: any, index: number) => {
            const HourIcon = hour.icon;
            const isCurrentHour = new Date().getHours() === parseInt(hour.time.split(':')[0]);
            return (
              <div key={index} className={`relative flex flex-col items-center gap-2 flex-shrink-0 text-center w-24 p-2 rounded-lg transition-colors ${isCurrentHour ? 'bg-emerald-100 dark:bg-emerald-900/50' : 'hover:bg-slate-100 dark:hover:bg-slate-700/50'}`}>
                <p className={`font-semibold ${isCurrentHour ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-600 dark:text-slate-300'}`}>{hour.time}</p>
                <HourIcon size={28} className={isCurrentHour ? 'text-emerald-500' : ''} />
